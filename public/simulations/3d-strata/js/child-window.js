...
class TR {
    constructor(n) {
        ...
        this.td1Input = createInput(0, "number").id("td1Input" + num).parent("td1" + num).class("form-control")
        this.td2 = createElement("td").id("td2" + num).parent("tr" + num)
        this.td2Input = createInput(0, "number").id("td2Input" + num).parent("td2" + num).class("form-control")
        this.td3 = createElement("td").id("td3" + num).parent("tr" + num)
        this.td3Select = createSelect().id("td3Select" + num).parent("td3" + num).class("form-select")
        ...
        function _removeButtonFunction() {
            ...
            trArr.pop(num)
            trNum -= 1
            idArr.splice(idArr.findIndex(idIndex => idIndex == str(num)), 1)
            for (let i = 0; i < idArr.length; i++) {
                select("#th" + idArr[i]).html(i + 1 + "層目")
            }
        }

        this.trRemoveButton = createButton("削除").parent("td4" + num).class("btn btn-outline-danger w-100").id("trRemoveButton" + num).mousePressed(_removeButtonFunction)
